     ...
     process.nextTick(function(){
          data.find({
            ...
            for(var i = 0; i < num_new_stocks; i++) {
              symbol = stocks[i]['Symbol']; console.log(symbol)
              eps_qq = stocks[i]['eps_qq']; console.log(eps_qq)
              price = stocks[i]['price']; console.log(price)
...
function getRHdata(a,b,callback){
     var Robinhood = require('robinhood')(credentials, function(){
       Robinhood.instruments(symbol,function(err, response, body){
         if(err){
          ...
          console.log(quoteUrl);
          var rhObject = {'rhUrl':rhUrl,'quoteUrl':quoteUrl}
...
function updateMongo(a,b,callback){
     trades.find(({}), function (err, docs) {
      trades.updateOne({'_id':docs[0]['_id']}, {$set:{'trade_id':trade_id, 'updated':new Date(), 'quote_url': rhUrl }}, function(err, res) {
         if (err) {
